...
fn setup() -> Mollusk {
    Mollusk::new(&quasar_test_sysvar::ID, "../../target/deploy/quasar_test_sysvar")
}
...
fn test_read_clock_syscall() {
    ...
    let instruction: Instruction = ReadClockInstruction { payer, snapshot, system_program }.into();
    let result = mollusk.process_instruction(&instruction, &[
        (payer, payer_account),
        (snapshot, snapshot_account),
        (system_program, system_program_account),
    ]);
    assert!(result.program_result.is_ok(), "read_clock failed: {:?}", result.program_result);
    ...
    println!("  read_clock (syscall): OK (CU: {})", result.compute_units_consumed);
}
...
fn test_read_clock_default_slot() {
    ...
    assert!(result.program_result.is_ok(), "read_clock default failed: {:?}", result.program_result);
    let data = &result.resulting_accounts[1].1.data;
    let slot = u64::from_le_bytes(data[1..9].try_into().unwrap());
    assert_eq!(slot, 0, "default slot should be 0");
    println!("  read_clock (default): OK (CU: {})", result.compute_units_consumed);
}
...
fn test_read_rent_syscall() {
    ...
    let snapshot_account = Account::default();
    let instruction: Instruction = ReadRentInstruction { payer, snapshot, system_program }.into();
    let result = mollusk.process_instruction(&instruction, &[
        (payer, payer_account),
        (snapshot, snapshot_account),
        (system_program, system_program_account),
    ]);
    assert!(result.program_result.is_ok(), "read_rent failed: {:?}", result.program_result);
    let data = &result.resulting_accounts[1].1.data;
    ...
    assert!(min_balance > 0, "min_balance for 100 bytes should be > 0, got {}", min_balance);
    println!("  read_rent (syscall): OK (CU: {}, min_balance_100={})", result.compute_units_consumed, min_balance);
}
...
fn test_read_clock_from_account() {
    ...
    let instruction: Instruction = ReadClockFromAccountInstruction { _payer: payer, snapshot, clock }.into();
    let result = mollusk.process_instruction(&instruction, &[
        (payer, payer_account),
        (snapshot, snapshot_account),
        (clock, clock_account),
    ]);
    assert!(result.program_result.is_ok(), "read_clock_from_account failed: {:?}", result.program_result);
    let data = &result.resulting_accounts[1].1.data;
    let slot = u64::from_le_bytes(data[1..9].try_into().unwrap());
    assert_eq!(slot, 100, "slot should be 100 after warp_to_slot(100)");
    println!("  read_clock (account): OK (CU: {})", result.compute_units_consumed);
}
...
fn test_read_clock_account_after_warp() {
    ...
    assert!(result.program_result.is_ok(), "read_clock_from_account after warp failed: {:?}", result.program_result);
    let data = &result.resulting_accounts[1].1.data;
    let slot = u64::from_le_bytes(data[1..9].try_into().unwrap());
    assert_eq!(slot, 999, "slot should be 999");
    println!("  read_clock (account, warp=999): OK (CU: {})", result.compute_units_consumed);
}
